    ...
    public void init() {
        try {
            if (apiKey != null && !apiKey.equals("YOUR_GOOGLE_CLOUD_API_KEY") && !apiKey.isEmpty()) {
                ...
                System.out.println("Google Cloud Translation API initialized successfully.");
            } else {
                System.out.println("Google Cloud Translation API Key not found. Using Mock Translation.");
            }
        } catch (Exception e) {
            System.err.println("Failed to initialize Google Translation API: " + e.getMessage());
        }
    }
